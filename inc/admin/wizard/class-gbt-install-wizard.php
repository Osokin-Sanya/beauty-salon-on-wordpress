...
if ( ! class_exists( 'GBT_InstallWizard' ) ) {
	...
	class GBT_InstallWizard {
		...
		public function setupWizard() {
			...
			wp_enqueue_script( 'gbt-wizard-js', get_template_directory_uri() .'/inc/admin/wizard/js/wizard.js', array( 'jquery' ), shopkeeper_theme_version() );
		...
		public function gbt_setup_demo() {
			...
			<div class="wrapper wizard-demo-import">
				...
				<div class="content-info">
					...
					<p><?php esc_html_e( 'Start with pre-built page layouts, dummy product pages, blog posts and widgets.', 'shopkeeper');?></p>
					<p><strong><?php esc_html_e( 'To start the import process, click "Install", take a little break, then come back here in 15 to 30 minutes.', 'shopkeeper');?></strong></p>
					<p><em><?php esc_html_e( 'Please be patient and don’t refresh this page, the import process may take a while, this also depends on your server.', 'shopkeeper');?></em></p>
				</div>
				...
				<div class="demo-icon <?php echo ( ! $is_required_plugins === true) ? 'error' : '' ?>">
					...
					<?php else : ?>
						<p class="error-info"><?php esc_html_e( 'Please make sure WPBakery Page Builder and WooCommerce are installed and activated before importing the demo content.', 'shopkeeper' ); ?></p>
					<?php endif; ?>
				</div>
